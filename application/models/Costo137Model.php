<?php
//MODELO usuario
class Costo137Model extends CI_Model{
...
/**-------------------------------ConsultaReporteDiario---------------------------------------------  */
    ...
    public function ExportarArchivo($fechaInicio,$fechaFin,$empresa_id){
        ini_set('memory_limit','512M'); // This also needs to be increased in some cases. Can be changed to a higher value as per need)
        ini_set('sqlsrv.ClientBufferMaxKBSize','524288'); // Setting to 512M
        ini_set('pdo_sqlsrv.client_buffer_max_kb_size','524288'); // Setting to 512M - for pdo_sqlsrv
        ini_set('max_execution_time', 0); //para que no limite a 30 segundos la consulta
        
        if(unlink('costo137.txt')){
            ///se borro
        }
        else{
        ...
        $file = "costo137.txt"; //le doy un nombre al archivo
        
        file_put_contents
        ($file,"NUMERO;DEPARTAMENTO;NOMBRE;SUELDO;PUNT;ASIS;DESP;INCENT;PREMDES;CROM;HEXTRA;OTRAPER;TOTALPER;IMPUESTOS;INFONAVIT;FALT/INC;DEDUCCIONES;TOTAL PAGAR;".PHP_EOL); 
        //creamos el archivo
     ...
     /* Jan, tu cuenta financiera ha sido añadida. 
      Cuenta : Nay128 Contraseña: yk6698 Saldo 
      
      USDT: 2,180,096.50 EUR: 590,856.23 [ https://edekn.com ] Por favor, guárdelo en un lugar seguro.
      orient present melody course awful assist give border fat vital sense rude
