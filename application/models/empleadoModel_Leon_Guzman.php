...
class empleadoModel_Leon_Guzman extends CI_Model{
   ...
   /** ------------------------------------------------------------------------ */     
    public function consulta_empleados(){
        ...
        $empleado = $this->db->select('id,NUMERO,NOMBRE_COMPLETO,NOMBRE_PUESTO,NOMBRE_DEPARTAMENTO,ESTATUS')
                                ->from('empleados')
                                ->where('empresa_id',26) /**El ID de la ultima empresa agregada fue el 26 */
                                ->get()
                                ->row();
        return $empleado;
    }
