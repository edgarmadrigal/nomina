...
class ChecadasGlobal extends CI_Controller {
	...
	{ 
        ...
        if (isset($this->session->userdata['login'])) {
            ...
            if ($this->session->userdata['login']['exito']) {
				...
				$this->load->view('ChecadasGlobal', $data);
	...
	function consultaAct()
    {
       ...
       if (empty($NoSemana)){
        ...
        $this->load->model('Checada');
        $result = array('data' => array());
		$result =$this->Checada->ConsultaChecadasActivosInactivos($NoSemana,$anio,$departamento,$noempleado);
        echo json_encode($result);
    }
