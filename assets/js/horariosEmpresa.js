...
function editar(id) {
    Modulo.isEditing = true;
    Modulo.editMeta = { id: id };
    $('#id').val(id);
    Modulo.fnCargarSelectsModal(id);
}
function eliminar(id_delete,horario,dia){ 
    var varURL = Modulo.baseurl + "horariosEmpresa/consultaHorario"; 
    $.ajax({
        url: varURL,
        data: {
            empresa_id: parseInt($('#id_empresa').val()),
            dia:parseInt(dia)
        },
        type: 'post',
    }).done(function (data) {        
        ...
        if(Modulo.dtValidar.length>1){
            ...
            .then((willDelete) => {
                if (willDelete) {
                    $.ajax({
                        url:  Modulo.baseurl+'horariosEmpresa/delete',
                        type: 'post',
                        dataType: 'json',
                        data: {
                            id: id_delete
                        ...
                        if (data) {
                            ...
                            Modulo.fnObtenerDatos($('#id_empresa').val());   
                        } else {                            
                            // update the manageMemberTable
                            Modulo.fnObtenerDatos($('#id_empresa').val());   
                        }
...
Modulo = {
    ...
    , fnConstruirTabla: function () {
        ...
        Modulo.fnMostrarSoloDiaActivo();
        $(".loadTable").css("display", "none");
        
    }
    , fnMostrarSoloDiaActivo: function () {
        var dayMap = {
            '#lunestab': '#tablaLunes',
            '#martestab': '#tablaMartes',
            '#miercolestab': '#tablaMiercoles',
            '#juevestab': '#tablaJueves',
            '#viernestab': '#tablaViernes',
            '#sabadotab': '#tablaSabado',
            '#domingotab': '#tablaDomingo'
        };
        var activeTab = $('.nav-tabs .nav-link.active').attr('href');
        var selector = dayMap[activeTab] || '#tablaLunes';
        $('#tablaLunes, #tablaMartes, #tablaMiercoles, #tablaJueves, #tablaViernes, #tablaSabado, #tablaDomingo').hide();
        $(selector).show();
    }
    //Carga los selects
    , fnCargarSelectsModal(id){
        $('#horarioLunes').empty();  
        $.ajax({
            url:  Modulo.baseurl+'horariosEmpresa/consultaHorarios',
            type: 'post',
            dataType: 'json'
        }).done(function (data) {
        ...
        $.ajax({
            ...
            $('#id').val(id);
            $.ajax({
                url: Modulo.baseurl+'horariosEmpresa/consultaHorarioEditar',
                type: 'post',
                dataType: 'json',
                data: {
                    id: id
                ...
                if (data && data.length > 0) {            
                    $('#id').val(id);
                    Modulo.editMeta.dia = parseInt(data[0].dia, 10);
                    Modulo.editMeta.originalIdHorario = data[0]['idhorario' + (data[0].dia === 1 ? 'Lunes' : data[0].dia === 2 ? 'Martes' : data[0].dia === 3 ? 'Miercoles' : data[0].dia === 4 ? 'Jueves' : data[0].dia === 5 ? 'Viernes' : data[0].dia === 6 ? 'Sabado' : 'Domingo')];
...
$(document).ready(function () {
    ...
    $('#horarioLunes').change(function() {
        ...
        if(valueSelected!=1) {
            ...
            $.ajax({
                url: Modulo.baseurl+'horariosEmpresa/consultaHorarioID',
                data: {
                    id: valueSelected
                },
                type: 'post',
    ...
    $(document).on('click', '#btn_guardar', function (e) {
        e.preventDefault();                       
        if(validar()){
            var id = parseInt($('#id').val().trim(), 10) || 0;
            if($('#fechaAsignacion').val()!=''){
                if(Modulo.isEditing && id > 0){
                    ///GUARDAR EDICION
                    $.ajax({
                        url: Modulo.baseurl+'horariosEmpresa/ActualizarHorariosEmpresa',
                        type: 'post',
                        dataType: 'json',
                        data: {
                            'id'                    : $('#id').val().trim(),
                            'empresa_id'            : $('#id_empresa').val().trim(),
                        ...
                        if (data) {
                            ...
                            swal({
                                ...
                                .then((value) => {
                                    window.location.assign( Modulo.baseurl+'horariosEmpresa');
                                });
                        }
                    });
                }else{        
                    ///GUARDAR NUEVO  
                    $.ajax({
                        url: Modulo.baseurl+'horariosEmpresa/AgregarHorariosEmpresa',
                        type: 'post',
                        dataType: 'json',
                        data: {
                            'empresa_id'           : $('#id_empresa').val().trim(),
